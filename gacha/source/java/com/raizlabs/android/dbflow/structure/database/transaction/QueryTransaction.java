...
public class QueryTransaction<TResult extends Model> implements ITransaction {
    ...
    public void execute(DatabaseWrapper databaseWrapper) {
        final CursorResult queryResults = this.modelQueriable.queryResults();
        if (this.queryResultCallback != null) {
            if (this.runResultCallbacksOnSameThread) {
                this.queryResultCallback.onQueryResult(this, queryResults);
            } else {
                Transaction.getTransactionHandler().post(new Runnable() {
                    public void run() {
                        QueryTransaction.this.queryResultCallback.onQueryResult(QueryTransaction.this, queryResults);
                    }
                });
            }
        }
        if (this.queryResultListCallback != null) {
            final List toListClose = queryResults.toListClose();
            if (this.runResultCallbacksOnSameThread) {
        ...
        if (this.queryResultSingleCallback != null) {
            final Model toModelClose = queryResults.toModelClose();
            if (this.runResultCallbacksOnSameThread) {
                this.queryResultSingleCallback.onSingleQueryResult(this, toModelClose);
            } else {
                Transaction.getTransactionHandler().post(new Runnable() {
                    public void run() {
                        QueryTransaction.this.queryResultSingleCallback.onSingleQueryResult(QueryTransaction.this, toModelClose);
                    }
                });
            }
        }
