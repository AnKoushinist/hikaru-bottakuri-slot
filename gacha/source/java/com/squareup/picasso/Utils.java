...
final class Utils {
    ...
    static boolean parseResponseSourceHeader(String str) {
        ...
        try {
            if (!("CONDITIONAL_CACHE".equals(split[0]) && Integer.parseInt(split[1]) == HttpResponseCode.NOT_MODIFIED)) {
                z = false;
            }
            return z;
    ...
    static Downloader createDefaultDownloader(Context context) {
        ...
        if (obj2 == obj) {
            return obj2 != null ? OkHttpLoaderCreator.create(context) : new UrlConnectionDownloader(context);
        } else {
            throw new RuntimeException("Picasso detected an unsupported OkHttp on the classpath.\nTo use OkHttp with this version of Picasso, you'll need:\n1. com.squareup.okhttp:okhttp:1.6.0 (or newer)\n2. com.squareup.okhttp:okhttp-urlconnection:1.6.0 (or newer)\nNote that OkHttp 2.0.0+ is supported!");
        }
    }

    static void sneakyRethrow(Throwable th) {
        sneakyThrow2(th);
    }
    ...
    static boolean isWebPFile(InputStream inputStream) {
        ...
        if (inputStream.read(bArr, 0, WEBP_FILE_HEADER_SIZE) == WEBP_FILE_HEADER_SIZE && WEBP_FILE_HEADER_RIFF.equals(new String(bArr, 0, 4, "US-ASCII")) && WEBP_FILE_HEADER_WEBP.equals(new String(bArr, 8, 4, "US-ASCII"))) {
            return true;
        }
        return false;
    }
