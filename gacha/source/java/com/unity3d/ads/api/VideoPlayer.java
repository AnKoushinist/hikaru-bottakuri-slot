...
public class VideoPlayer {
    ...
    @WebViewExposed
    public static void prepare(final String str, final Double d, WebViewCallback webViewCallback) {
        DeviceLog.debug("Preparing video for playback: " + str);
        Utilities.runOnUiThread(new Runnable() {
            public void run() {
                if (VideoPlayer.getVideoPlayerView() != null) {
                    VideoPlayer.getVideoPlayerView().prepare(str, Float.valueOf(d.floatValue()));
                }
            }
        });
        if (getVideoPlayerView() != null) {
    ...
    public static void setInfoListenerEnabled(boolean z, WebViewCallback webViewCallback) {
        if (VERSION.SDK_INT <= 16) {
            webViewCallback.error(VideoPlayerError.API_LEVEL_ERROR, Integer.valueOf(VERSION.SDK_INT), Integer.valueOf(17), Boolean.valueOf(z));
